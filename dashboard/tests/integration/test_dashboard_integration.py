...
Funções do conjunto:
    ...
    get_program_summary (selectors)      — agregação por programa
    get_cost_composition (selectors)     — split materiais vs horas
    _normalize_dashboard_filters (views) — normaliza query params
    ...
    SummaryTableView (views.py)          — endpoint GET /api/main-dashboard/summary/
    CostCompositionView (views.py)       — endpoint GET /api/main-dashboard/composition/

Estes testes usam PostgreSQL real (via settings_integration).
Os selectors usam SQL raw com schemas silver.*, que exigem PostgreSQL.
"""
...
class TestDashboardKPIsIntegration:
    ...
    Valida que os KPIs refletem exatamente o que está no banco.
    """
    ...
    def test_custo_materiais_reflete_soma_real_do_banco(
        self, projeto, pedido_compra
    ):
        SilverSolicitacaoCompra.objects.create(
            id=1,
            projeto=projeto,
            silver_ingested_at=datetime.now(tz=timezone.utc),
        )
        SilverComprasProjeto.objects.create(
            id=1,
            solicitacao=SilverSolicitacaoCompra.objects.get(id=1),
    ...
    def test_filtro_por_programa_isola_dados(self, programa, projeto, pedido_compra):
        # Programa diferente
    ...
    def test_custo_horas_reflete_horas_trabalhadas_reais(self, projeto, tarefa):
        ...
        # custo_hora do projeto = 200.0, horas = 18 → custo = 3600
...
class TestTopProjectsIntegration:
    ...
    def test_top_projects_limita_a_10_resultados(self, programa, pedido_compra):
        for i in range(15):
...
class TestCostEvolutionIntegration:
    ...
    def test_filtro_por_data_retorna_apenas_periodo_correto(
        self, programa, projeto, pedido_compra
    ):
        sol = SilverSolicitacaoCompra.objects.create(
            id=501, projeto=projeto, silver_ingested_at=datetime.now(tz=timezone.utc)
        )

        # Compra dentro do período
        pc_jan = SilverPedidoCompra.objects.create(
            id=501, numero_pedido="PC-501", data_pedido=date(2024, 1, 15),
            silver_ingested_at=datetime.now(tz=timezone.utc),
        )
        # Compra fora do período
        pc_dez = SilverPedidoCompra.objects.create(
            id=502, numero_pedido="PC-502", data_pedido=date(2024, 12, 15),
            silver_ingested_at=datetime.now(tz=timezone.utc),
        )
