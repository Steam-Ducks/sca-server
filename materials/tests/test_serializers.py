...
def test_materials_serializer_retorna_dados_corretos():
    ...
    programa = SilverPrograma.objects.create(
        id=1, codigo_programa="PROG-001", nome_programa="Programa Alpha", silver_ingested_at=now
    )
    projeto = SilverProjeto.objects.create(
        id=1, codigo_projeto="PROJ-001", nome_projeto="Projeto Alpha", programa=programa, silver_ingested_at=now
    )
    material = SilverMaterial.objects.create(
        id=1, codigo_material="MAT-001", descricao="Cabo de aço", categoria="Estrutural", custo_estimado=150.00, silver_ingested_at=now
    )
    fornecedor = SilverFornecedor.objects.create(
        id=1, codigo_fornecedor="FORN-001", razao_social="Fornecedor Ltda", silver_ingested_at=now
    )
