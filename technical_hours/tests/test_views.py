...
def test_technical_hours_table_returns_200(api_client):
    with patch.object(TechnicalHoursTableView, "get_queryset", return_value=[]):
        response = api_client.get("/api/horas-tecnicas/")
        assert response.status_code == 200


def test_technical_hours_table_returns_list(tempo_em_memoria, api_client):
    with patch.object(
...
def test_technical_hours_table_retorna_campos_corretos(tempo_em_memoria, api_client):
    with patch.object(
        TechnicalHoursTableView, "get_queryset", return_value=[tempo_em_memoria]
    ):
        response = api_client.get("/api/horas-tecnicas/")
...
def test_technical_hours_table_lista_vazia_retorna_200(api_client):
    with patch.object(TechnicalHoursTableView, "get_queryset", return_value=[]):
        response = api_client.get("/api/horas-tecnicas/")
        assert response.status_code == 200
...
def test_filtro_por_periodo_yyyy_mm_retorna_200(tempo_em_memoria, api_client):
    with patch.object(
        TechnicalHoursTableView, "get_queryset", return_value=[tempo_em_memoria]
    ):
...
def test_filtro_por_ano_retorna_200(tempo_em_memoria, api_client):
    with patch.object(
        TechnicalHoursTableView, "get_queryset", return_value=[tempo_em_memoria]
    ):
        response = api_client.get(f"{URL}?ano=2024")
        assert response.status_code == 200
        assert len(response.data) == 1


def test_filtro_por_mes_retorna_200(tempo_em_memoria, api_client):
    with patch.object(
        TechnicalHoursTableView, "get_queryset", return_value=[tempo_em_memoria]
    ):
        response = api_client.get(f"{URL}?mes=3")
        assert response.status_code == 200
        assert len(response.data) == 1


def test_filtro_por_ano_e_mes_retorna_200(tempo_em_memoria, api_client):
    with patch.object(
        TechnicalHoursTableView, "get_queryset", return_value=[tempo_em_memoria]
    ):
...
def test_periodo_com_tres_partes_retorna_400(api_client):
    response = api_client.get(f"{URL}?periodo=2024-03-15")
    assert response.status_code == 400
    assert "periodo" in response.data


def test_ano_nao_numerico_retorna_400(api_client):
    response = api_client.get(f"{URL}?ano=abc")
    assert response.status_code == 400
    assert "ano" in response.data


def test_mes_nao_numerico_retorna_400(api_client):
    response = api_client.get(f"{URL}?mes=abc")
    assert response.status_code == 400
    assert "mes" in response.data


def test_sem_filtro_retorna_todos_registros(tempo_em_memoria, api_client):
    with patch.object(
        TechnicalHoursTableView, "get_queryset", return_value=[tempo_em_memoria]
    ):
