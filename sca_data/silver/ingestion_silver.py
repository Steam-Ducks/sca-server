...
def _to_str(series: pd.Series, max_len: int = None) -> pd.Series:
    s = series.where(series.notna(), None).astype(str).str.strip()
    s = s.replace({"nan": None, "None": None, "<NA>": None})
    if max_len:
        s = s.str[:max_len]
    return s
...
def _transform_solicitacoes_compra(engine):
    try:
        ...
        logging.error(f"It was not possible to transform 'solicitacoes_compra'. Error: {e}")
...
def _transform_empenho_materiais(engine):
    try:
        ...
        logging.error(f"It was not possible to transform 'empenho_materiais'. Error: {e}")
...
def _transform_estoque_materiais_projeto(engine):
    try:
        ...
        logging.error(f"It was not possible to transform 'estoque_materiais_projeto'. Error: {e}")


# --------------------------------------------------------------------------- #
# Pipeline — ordem respeita dependências de FK
# --------------------------------------------------------------------------- #
