...
class TestGoldCostsTableIntegration:
    ...
    def test_lista_retorna_200(self, api_client):
        ...
        response = api_client.get("/api/costs/")
    ...
    def test_retorna_custo_real_do_banco(self, api_client, custo_gold):
    ...
    def test_data_serializada_em_formato_iso(self, api_client, custo_gold):
    ...
    def test_filtro_por_nome_programa(self, api_client, dois_custos):
        ...
        response = api_client.get("/api/costs/?nome_programa=MANSUP")

        assert response.status_code == 200
        assert len(response.data) == 1
        assert response.data[0]["nome_programa"] == "MANSUP"

    def test_filtro_por_nome_projeto(self, api_client, dois_custos):
        # CTI-06 (mínimo): ?nome_projeto= → response contém só o projeto filtrado
        response = api_client.get("/api/costs/?nome_projeto=Proj INFRA")

        assert response.status_code == 200
        assert len(response.data) == 1
        assert response.data[0]["nome_projeto"] == "Proj INFRA"

    def test_filtro_por_intervalo_de_datas(self, api_client, dois_custos):
        # CTI-07 (adicional): ?data_gte + ?data_lte → só registros no intervalo
        # Valida: parse_date + make_aware + filter(data__gte/lte) na view
        response = api_client.get("/api/costs/?data_gte=2024-01-01&data_lte=2024-03-31")
